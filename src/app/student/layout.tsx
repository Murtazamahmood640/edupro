...
import {
  GraduationCap, LayoutDashboard, BookOpen, Search, Brain, Award, Calendar,
  User, Bell, ChevronLeft, ChevronRight, LogOut, Settings, Menu
} from "lucide-react";

const navItems = [
  { label: "Dashboard", href: "/student/dashboard", icon: LayoutDashboard },
  { label: "My Courses", href: "/student/courses", icon: BookOpen },
  { label: "Browse Courses", href: "/student/browse", icon: Search },
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      {!collapsed && (
        <div className="px-5 py-4 border-b border-slate-700">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-edu-amber flex items-center justify-center font-bold text-white text-sm flex-shrink-0">J</div>
            <div className="overflow-hidden">
              <p className="text-white font-bold text-sm truncate">John Doe</p>
              <span className="inline-flex items-center px-2 py-0.5 bg-edu-indigo/20 text-indigo-300 text-xs rounded-full font-medium">Student</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={`px-3 py-4 border-t border-slate-700 space-y-1 ${collapsed ? "flex flex-col items-center" : ""}`}>
        <Link href="/student/profile" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:bg-slate-700/50 hover:text-white transition-all ${collapsed ? "justify-center" : ""}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex-shrink-0 h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-9 h-9 rounded-full bg-edu-amber flex items-center justify-center font-bold text-white text-sm">J</div>
          </div>
        </header>
